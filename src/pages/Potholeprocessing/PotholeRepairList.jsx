...
const PotholeRepairList = () => {
  const [sido, setSido] = useState('');
  const [sigungu, setSigungu] = useState('');
  const [selectedPothole, setSelectedPothole] = useState(null);

  const fetchPotholes = async (page) => {
    const response = await axios.get('/managements/reports', {
  ...
  const handleStatusChange = async (id, status) => {
    try {
      await axios.post(`/managements/reports/${id}`);
        ...
        <div style={{ height: '80vh', overflow: 'auto' }}>
          {data.map((pothole) => (
            <PotholeProcessItem
              ...
              firstButtonLabel="보수 완료"
              secondButtonLabel="보수 미완료"
              onButtonClick={(status) => handleStatusChange(pothole.id, status)}
            />
          ))}
...
export default PotholeRepairList;
